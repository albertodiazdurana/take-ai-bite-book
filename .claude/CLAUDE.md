...
- Build output: `book/_build/` (generated by `mystmd build`)
...
DSM Central at a tagged release, runs MyST to build a documentation site, and
deploys it to GitHub Pages. Tracked artifacts are code (`scripts/`, workflows,
`book/myst.yml`, `package.json` + lockfile); tracked content is governance
(`dsm-docs/`). The built markdown content (`book/content/`) is gitignored and
ephemeral per build.
